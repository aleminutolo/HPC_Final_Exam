...
# Output file
OUTPUT_FILE="../csvs/omp_strong_scaling.csv"
...
for THREADS in {1..128}; do
    ...
    # Execution
    EXEC_TIME=$(mpirun -np 1 --map-by socket --bind-to socket ./mandelbrot $n $n $X_LEFT $Y_LOWER $X_RIGHT $Y_UPPER $MAX_ITERATIONS $THREADS)
